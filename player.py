...
def create_input_vector(screen_width, screen_height, obstacles, player_x, player_y):
    if len(obstacles) > 0:
        new_obstacles = []
        for obstacle in obstacles:
            if obstacle['y'] < player_y:
                new_obstacles.append(obstacle)
        new_obstacles.sort(key=lambda x: np.abs(x['y'] - player_y), reverse=False)
        try:
            if len(obstacles) > 2:

                two_obstacles =[]
                for obstacle_index in range(len(new_obstacles)):
                    if len(two_obstacles) == 2:
                        break
                    if player_y - new_obstacles[obstacle_index]['y']  <= 0:
                        continue
                    elif  player_y - new_obstacles[obstacle_index]['y'] > 0:
                        two_obstacles.append(new_obstacles[obstacle_index])
                distance_from_nearest_obstacle = 1 - np.dot([player_x, player_y],
                                                                [two_obstacles[0]['x'], two_obstacles[0]['y']]) / \
                                                     (np.linalg.norm(
                                                         [two_obstacles[0]['x'],
                                                          two_obstacles[0]['y']]) * np.linalg.norm(
                                                         [player_x, player_y]))
                distance_from_second_nearest_obstacle = 1 - np.dot([player_x, player_y],
                                                    [two_obstacles[1]['x'], two_obstacles[1]['y']]) / \
                                         (np.linalg.norm(
                                             [two_obstacles[1]['x'],
                                              two_obstacles[1]['y']]) * np.linalg.norm(
                                             [player_x, player_y]))
                x = np.array(
                    ([distance_from_nearest_obstacle], [distance_from_second_nearest_obstacle], [player_x / screen_width],
                ...
                distance_from_nearest_obstacle = 1 - np.dot([player_x, player_y],
                                                                [new_obstacles[0]['x'], new_obstacles[0]['y']]) / \
                                                     (np.linalg.norm(
                                                         [new_obstacles[0]['x'],
                                                          new_obstacles[0]['y']]) * np.linalg.norm(
                                                         [player_x, player_y]))
                x = np.array(
                    ([distance_from_nearest_obstacle], [1], [player_x / screen_width],
                     [player_y / screen_height]))
                return x
...
class Player(pygame.sprite.Sprite):
    ...
    def think(self, screen_width, screen_height, obstacles, player_x, player_y):
        ...
        """
        x = create_input_vector(screen_width, screen_height, obstacles, player_x, player_y)
        try:
            result = self.nn.forward(x)
            # self.nn.forward(self.change_input_vector_of_nn(screen_width, screen_height, obstacles, player_x, player_y))
