        ...
        if game_active:
            ...
            if no_players_left:
                ...
                if game_mode == "Manual":
                    ...
                    evolution.save_fitness_result(int(np.min(fitnesses)),int(np.max(fitnesses)),
                                                  int(np.mean(fitnesses)))
                    with open('fitness_results.json', 'r') as in_file:
                        fitness_results = json.load(in_file)
                    plt.title('In the name of Almighty Allah')
                    plt.ylabel('Fitness')
                    plt.plot(fitness_results['min_fitness'],label='Min fitness')
                    plt.plot(fitness_results['max_fitness'],label='Max fitness')
                    plt.plot(fitness_results['mean_fitness'],label='Mean fitness')
                    plt.legend()

                    plt.pause(0.05)
                    print(f"prev_players:{fitnesses}")
            ...
            draw_intro_text("Seyed Moein Ayyoubzadeh", height=700, color='#AB8CD5')
